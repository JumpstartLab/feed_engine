class Tweet < ActiveRecord::Base
  ...
  def self.import_posts(user_id)
    ...
    Twitter.user_timeline(params).reverse.each do |tweet|
      tweet = user.feed.tweets.create(content: tweet.text, source_id: tweet.id, handle: tweet.user.screen_name, tweet_time: tweet.created_at)
      link_to_poly_post(tweet, user.feed)
    end
  end
