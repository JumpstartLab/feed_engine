class Instagramimage < ActiveRecord::Base
  ...
  def self.import_posts(user_id)
    ...
    call = "/v1/users/#{user.instagram_id}/media/recent/?access_token="
    call = call + "#{user.instagram_token}"
  ...
  def self.create_images(mash, user)
    ...
    mash.data.reverse.each do |gram|
      ...
      image_in_system = Instagramimage.find_by_source_id(gram.caption.id)
      if image_in_system.blank? && post_time > sign_up_time
        image = Instagramimage.create(
          content: gram.images.standard_resolution.url,
