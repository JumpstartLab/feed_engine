...
describe 'api/v1/user_tweets', type: :api do
  ...
  context "Create tweet" do
    describe "when no nickname is passed in" do
      before(:each) do
      ...
      it "returns a 500 error" do
        ...
        last_response.body.should == "Twitter account can not be found."
      end
    end

    context "When tweet has valid parameters" do
      before(:each) do
        post "#{url}.json",  tweets: [ {:comment => "hello" } ].to_json, token: user.authentication_token, nickname: "test"
      end
